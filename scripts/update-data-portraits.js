// Replace all Wikimedia portrait URLs in data.js with local /assets/portraits/ paths
// Run from project root: node scripts/update-data-portraits.js
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const DATA_JS = path.join(__dirname, '..', 'api', 'src', 'data.js');
const PORTRAITS_DIR = path.join(__dirname, '..', 'website', 'assets', 'portraits');

// Build URL → local path mapping by reading PORTRAITS object from download script
// (mirrors the PORTRAITS constant from download-portraits.js)
const PORTRAITS = {
  ...
  c75: 'https://upload.wikimedia.org/wikipedia/commons/thumb/d/dd/J_Balvin_BTTR_Tour_Photo_January_2025.jpg/330px-J_Balvin_BTTR_Tour_Photo_January_2025.jpg', // deleted — replaced locally
...
function getExt(url) {
  ...
  return base.toLowerCase().endsWith('.png') ? 'png' : 'jpg';
}
...
for (const [id, url] of Object.entries(PORTRAITS)) {
  ...
  const localFile = path.join(PORTRAITS_DIR, `${id}.${ext}`);
  if (!fs.existsSync(localFile) || fs.statSync(localFile).size < 5000) {
    console.log(`  SKIP ${id} — local file missing or too small`);
    continue;
  }
  const localPath = `/assets/portraits/${id}.${ext}`;
  // The URL appears in data.js wrapped in single or double quotes
  if (content.includes(url)) {
    content = content.replaceAll(url, localPath);
    replaced++;
    console.log(`  ✓ ${id} → ${localPath}`);
  } else {
    console.log(`  ? ${id} — URL not found in data.js`);
  }
}

fs.writeFileSync(DATA_JS, content, 'utf8');
console.log(`\n✓ Replaced ${replaced} URLs in data.js`);

// Also update the comment in data.js to reflect local hosting
const oldComment = '// ── Extended roster portraits — Wikipedia CDN real celebrity portraits (c31–c100) ──';
const newComment = '// ── Extended roster portraits — self-hosted local portraits (c31–c100) ──';
if (content.includes(oldComment)) {
  fs.writeFileSync(DATA_JS, fs.readFileSync(DATA_JS, 'utf8').replace(oldComment, newComment), 'utf8');
}
const oldNamedComment = '// id,  name,                 category,      agency,              region,           portrait (Wikipedia CDN),';
const newNamedComment = '// id,  name,                 category,      agency,              region,           portrait (local /assets/portraits/),';
if (fs.readFileSync(DATA_JS, 'utf8').includes(oldNamedComment)) {
  fs.writeFileSync(DATA_JS, fs.readFileSync(DATA_JS, 'utf8').replace(oldNamedComment, newNamedComment), 'utf8');
}
console.log('Done.');
